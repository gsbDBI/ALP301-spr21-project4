...
plot_diversity_recsys<-function(filename, ratings_matrix) {
  ...
  popularity_ones <- colSums(ratings_matrix[, index_results_one]>0.3)
  popularity_fives <- colSums(ratings_matrix[, index_results_five]>0.3)
  
  readers_per_story <- (colSums(ratings_matrix>0.3))
