...
source("../Utils/engagement_function_only.R", local = knitr::knit_global()) # Not used, see engagement code chunk below
```
...
```
```{r engagement plot}
hist(results$interacted, col="#e87d71", breaks=100, xlim=c(0, 0.007), ylim=c(0,70), border=F)
hist(results$recommended, col="#55bcc1", breaks=100, xlim=c(0, 0.007), ylim=c(0,70), border=F)
```
