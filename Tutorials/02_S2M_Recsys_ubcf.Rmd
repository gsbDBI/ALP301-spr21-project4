...
```{r generate rec_results_ubcf, eval=FALSE}
...
# If the file does not exist you have to start from the first user
# recommender_results_item <- Recommender_Results_Data_Frame(1, 10, utility_matrix, similarity_matrix_user)
# next_user = 2

# If the file already exists you can load it and start from the last user processed
recommender_results_item <- tibble(read_csv("Results/rec_results_ubcf.csv"),col_types = cols(.default = col_double()))
...
write.csv(recommender_results_item, "Results/rec_results_ubcf.csv", row.names = FALSE)
```

```{r load_rec_results, eval=FALSE}
