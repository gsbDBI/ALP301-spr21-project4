...
types <- c("cbf", "svd", "ibcf", "ubcf", "fm", "random")
...
for (type in c("cbf")) {
  ...
  cross_validation_recsys(utility_matrix=utility_matrix,
                                   folds=10, X=5, type=type, key="1", params=params[[type]],
...
for (k in 1:length(types)) {
  results_type <- read_csv(paste("../Results/top_5_10_fold_cv_",types[k],"_0.csv", sep = ""))
...
results0_df <- tibble(type=types, unknown_value="0.0", precision.at.5=precision_vector, recall.at.5=recall_vector, rmse=rmse_vector)

precision_vector<-rep(NA,length(types))
recall_vector<-rep(NA,length(types))
rmse_vector<-rep(NA,length(types))
for (k in 1:length(types)) {
  results_type <- read_csv(paste("../Results/top_5_10_fold_cv_",types[k],"_1.csv", sep = ""))
  precision_vector[k] <- mean(results_type$precision.at.5)
  recall_vector[k] <- mean(results_type$recall.at.5)
  rmse_vector[k] <- mean(results_type$rmse)
}
results1_df <- tibble(type=types, unknown_value="0.1", precision.at.5=precision_vector, recall.at.5=recall_vector, rmse=rmse_vector)

results <- rbind(results0_df, results1_df)
results$unknown_value <- as.factor(results$unknown_value)

ggplot(data = results, aes(x = precision.at.5, y = recall.at.5, group=unknown_value)) + 
  geom_point(mapping = aes(size=rmse, fill=type), alpha=0.5, pch=21) +
  facet_wrap(~ unknown_value)
```
